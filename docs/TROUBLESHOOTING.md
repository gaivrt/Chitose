...
这是 **URL 编码与文件系统编码不匹配**的问题。具体来说：

1. **浏览器**: 将中文字符转换为 URL 编码（UTF-8 字节的 %XX 形式）
   - `芊芊` → `%E8%8A%8A%E8%8A%8A`

2. **HTTP 服务器**: 接收 URL 编码的请求，需要解码为文件系统路径

3. **文件系统**: 根据操作系统和配置，可能使用不同的编码
   - Linux/Mac: 通常是 UTF-8
   - Windows: 可能是 GBK、UTF-8 或其他编码
   - WSL: 可能存在编码转换问题

4. **问题**: 当 URL 编码解码后的字节序列与文件系统实际使用的编码不匹配时，服务器就找不到文件，返回 404。

**简单来说**: 浏览器发送的中文字符编码和文件系统存储的编码可能不一致，导致文件找不到。
